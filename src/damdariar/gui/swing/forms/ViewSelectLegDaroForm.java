...
public class ViewSelectLegDaroForm extends AbstractForm {
	
    static int i = 1;
    
	public ViewSelectLegDaroForm() {
		super(ViewSelectLegDaro.class);
	}

	protected boolean beforeSave() {
		if(((ViewSelectLegDaro)($Form_Bean)).getCDaroTajvizId() == null){
		...
		if(list != null && list.size() > 0){
			...
			((ViewSelectLegDaro)($Form_Bean)).setCDaroTajvizId((max != null ? max : 0) + i);
			i++;
		}
		}
		return true;
	...
	public void befroeLayoutComponents() {
        editorsMap.remove("CCowLegStatusId");
