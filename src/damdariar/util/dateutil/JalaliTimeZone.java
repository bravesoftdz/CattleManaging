/*
 * @(#)JalaliTimeZone.java	1.00 01/06/04
 * based on SimpleTimeZone.java	1.38 00/01/19
 *
...
/**
 * <code>JalaliTimeZone</code> is a concrete subclass of <code>TimeZone</code>
 * that represents a time zone for use with a Jalali calendar, means that
 * expects Jalali dates in input.
 * <p/>
 * <P>
 * Use a negative value for <code>dayOfWeekInMonth</code> to indicate that
 * <code>JalaliTimeZone</code> should count from the end of the month backwards.
 * For example, Daylight Savings Time may end at the last
 * (dayOfWeekInMonth = -1) Friday in Shahrivar, at 0 AM.
 *
 * @author David Goldsmith, Mark Davis, Chen-Lieh Huang, Alan Liu, Behdad Esfahbod
 * @version 1.00
 * @see BECalendar
 * @see BEJalaliCalendar
 * @see TimeZone
 */
public class JalaliTimeZone extends TimeZone {
    /**
     * Constructs a JalaliTimeZone with the given base time zone offset from GMT
     ...
     */
    public JalaliTimeZone(int rawOffset, String ID) {
    ...
    /**
     * Construct a JalaliTimeZone with the given base time zone offset from
     ...
     * use JalaliTimeZone(rawOffset, ID).
     * <p/>
     * By default, this constructor specifies day-of-week-in-month rules. That
     * is, if the startDay is 1, and the startDayOfWeek is SATURDAY, then this
     * indicates the first Saturday in the startMonth. A startDay of -1 likewise
     * indicates the last Saturday. However, by using negative or zero values for
     * certain parameters, other types of rules can be specified.
     * <p/>
     * Day of month. To specify an exact day of the month, such as Farvardin 1, set
     ...
     * negative. For example, if startDay is 5 and startDayOfWeek is -FRIDAY,
     * this indicates the first Friday on or after the 5th day of the
     * startMonth.
     * <p/>
     ...
     * and startDayOfWeek is -THURSDAY, this indicates the last Thursday on or
     ...
     * Constructor.
     */
    JalaliTimeZone(int rawOffset, String ID,
     ...
     * Time may start at the first Saturday in Farvardin, at 2 AM.
     * Therefore, you can set the start rule by calling:
     * setStartRule(TimeFields.Farvardin, 1, TimeFields.SATURDAY, 2*60*60*1000);
     *
     ...
     * may end at the last (-1) Saturday in Shahrivar, at 2 AM.
     * Therefore, you can set the end rule by calling:
     * setEndRule(TimeFields.Shahrivar, -1, TimeFields.SATURDAY, 2*60*60*1000);
     *
     ...
     * uses a default {@link BEJalaliCalendar} object as its
     * underlying calendar, such as for determining leap years.  Do
     * not use the result of this method with a calendar other than a
     * default <code>BEJalaliCalendar</code>.
     * <p/>
     * <p><em>Note:  In general, clients should use
     * <code>BECalendar.get(ZONE_OFFSET) + BECalendar.get(DST_OFFSET)</code>
     ...
     * @throws IllegalArgumentException the era, month, day,
    ...
    public int getOffset(int era, int year, int month, int day, int dayOfWeek,
        ...
        // 7-argument getOffset(), and outside code should use BECalendar.get(int
        // field) with fields ZONE_OFFSET and DST_OFFSET. We can't get rid of
        // this method because it's public API. - liu 8/10/98
        if (month < BEJalaliCalendar.FARVARDIN
                || month > BEJalaliCalendar.ESFAND) {
            throw new IllegalArgumentException("Illegal month " + month);
        }
        int monthLength, prevMonthLength;
        if ((era == BEJalaliCalendar.AH) && internalCal.isLeapYear(year)) {
    ...
    int getOffset(int era, int year, int month, int day, int dayOfWeek,
        ...
        if (true) {
             ...
             * file.  */
            if ((era != BEJalaliCalendar.AH && era != BEJalaliCalendar.BH)
                    || month < BEJalaliCalendar.FARVARDIN
                    || month > BEJalaliCalendar.ESFAND
                    || day < 1
                    || day > monthLength
                    || dayOfWeek < BECalendar.SUNDAY
                    || dayOfWeek > BECalendar.SATURDAY
                    || millis < 0
                    || millis >= millisPerDay
                    || monthLength < 29
                    || monthLength > 31
            ...
            if (dayOfWeek < BECalendar.SUNDAY
                    || dayOfWeek > BECalendar.SATURDAY) {
        ...
        if (!useDaylight || year < startYear || era != BEJalaliCalendar.AH) return result;
    ...
    private static int compareToRule(int month, int monthLen, int prevMonthLen,
        ...
        while (millis >= millisPerDay) {
            ...
            if (dayOfMonth > monthLen) {
                 ...
                 * from ESFAND to ESFAND+1, since we use the result to
    ...
    public boolean inDaylightTime(Date date) {
        BEJalaliCalendar jc = new BEJalaliCalendar(this);
        jc.setTime(date);
        return jc.inDaylightTime();
    }
    ...
    /**
     * Compares the equality of two JalaliTimeZone objects.
     *
     * @param obj The JalaliTimeZone object to be compared with.
     * @return True if the given obj is the same as this JalaliTimeZone
     *         object; false otherwise.
     */
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (!(obj instanceof JalaliTimeZone))
            return false;

        JalaliTimeZone that = (JalaliTimeZone) obj;
    ...
    public boolean hasSameRules(TimeZone other) {
        ...
        if (!(other instanceof JalaliTimeZone)) return false;
        JalaliTimeZone that = (JalaliTimeZone) other;
     ...
     * between <code>BEJalaliCalendar.FARVARDIN</code> and
     * <code>BEJalaliCalendar.ESFAND</code> inclusive.  This value must not equal
     * <code>endMonth</code>.
     ...
     * <code>startDayOfWeek</code> of <code>BECalendar.SATURDAY</code> indicates the
     * first Saturday of <code>startMonth</code>.  Likewise, +2 would indicate the
     * second Saturday, and -1 the last Saturday.  A value of 0 is illegal.
     * </dd>
     * </ul>
     * <p>If <code>useDaylight</code> is false, this value is ignored.
     *
     ...
     * must be between <code>BECalendar.SUNDAY</code> and
     * <code>BECalendar.SATURDAY</code> inclusive.
     ...
     * between <code>BEJalaliCalendar.FARVARDIN</code> and
     * <code>BEJalaliCalendar.ESFAND+1</code>.  This value must not equal
     * <code>startMonth</code>.
     * <p>If <code>useDaylight</code> is false, this value is ignored.
     *
     * @serial
     ...
     * savings time ends, from 1 to 30, or 31, depending on the
     * <code>endMonth</code>.
     * </dd>
     * <dt><code>endMode != DOW_IN_MONTH</code></dt>
     * <dd>
     ...
     * <code>endDayOfWeek</code> of <code>BECalendar.SATURDAY</code> indicates the
     * first Saturday of <code>endMonth</code>.  Likewise, +2 would indicate the
     * second Saturday, and -1 the last Saturday.  A value of 0 is illegal.
     * </dd>
     * </ul>
     * <p>If <code>useDaylight</code> is false, this value is ignored.
    ...
    private static BEJalaliCalendar internalCal = new BEJalaliCalendar();
     ...
     * <dd>
     * Exact day of week; e.g., Farvardin 1.
     * </dd>
     * <dt><code>DOW_IN_MONTH_MODE</code></dt>
     * <dd>
     * Day of week in month; e.g., last Saturday in Farvardin.
     * </dd>
     * <dt><code>DOW_GE_DOM_MODE</code></dt>
     * <dd>
     * Day of week after day of month; e.g., Saturday on or after Farvardin 15.
     * </dd>
     * <dt><code>DOW_LE_DOM_MODE</code></dt>
     * <dd>
     * Day of week before day of month; e.g., Saturday on or before Farvardin 15.
     * </dd>
     * </dl>
    ...
    //       lastSat  the last Saturday in the month
    //       lastFri  the last Friday in the month
    //       Sat>=8   first Saturday on or after the eighth
    //       Sat<=25  last Saturday on or before the 25th
    ...
    // This is the format JalaliTimeZone objects take after construction or
     ...
     * really specific to BEJalaliCalendar, which does not use that month.
     * The range for time includes ONEDAY (vs. ending at ONEDAY-1) because the
     * end rule is an exclusive limit point.  That is, the range of times that
     * are in DST include those >= the start and < the end.  For this reason,
     * it should be possible to specify an end of ONEDAY in order to include the
     * entire day.  Although this is equivalent to time 0 of the following day,
     * it's not always possible to specify that, for example, on Esfand 30.
    ...
    private void decodeStartRule() {
        ...
        if (startDay != 0) {
            if (startMonth < BEJalaliCalendar.FARVARDIN || startMonth > BEJalaliCalendar.ESFAND) {
            ...
            if (startDayOfWeek == 0) {
                ...
                if (startDayOfWeek > BECalendar.SATURDAY) {
    ...
    private void decodeEndRule() {
        useDaylight = (startDay != 0) && (endDay != 0);
        if (endDay != 0) {
            if (endMonth < BEJalaliCalendar.FARVARDIN || endMonth > BEJalaliCalendar.ESFAND) {
            ...
            if (endDayOfWeek == 0) {
                ...
                if (endDayOfWeek > BECalendar.SATURDAY) {
    ...
    private void makeRulesCompatible() {
        switch (startMode) {
            case DOM_MODE:
                ...
                startDayOfWeek = BECalendar.SUNDAY;
                break;
            case DOW_GE_DOM_MODE:
        ...
        switch (endMode) {
            case DOM_MODE:
                endDay = 1 + (endDay / 7);
                endDayOfWeek = BECalendar.SUNDAY;
                break;
            case DOW_GE_DOM_MODE:
                // A day-of-month of 1 is equivalent to DOW_IN_MONTH_MODE
                // that is, Sun>=1 == firstSun.
    ...
    private void readObject(ObjectInputStream stream)
        ...
        if (serialVersionOnStream < 1) {
            ...
            // too much, so we assume SATURDAY, which actually works most of the time.
            if (startDayOfWeek == 0) startDayOfWeek = BECalendar.SATURDAY;
            if (endDayOfWeek == 0) endDayOfWeek = BECalendar.SATURDAY;
